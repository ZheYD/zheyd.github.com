...
var gameMonitor = {
	...
	getScore : function(){
		...
		var user = 1;
		$('.btn1').text('帅邓哲宇再给玩一次').removeClass('share').addClass('playagain');
		if(score==0){
			$('#scorecontent').html('真遗憾，您竟然<span class="lighttext">一个</span>月饼都没有抢到！');
			$('.btn1').text('美女请重新来过').removeClass('share').addClass('playagain');
			...
			$('.btn1').text('好厉害，亲亲~').removeClass('share').addClass('playagain');
			user = 10;
		}
		else if(score>20 && score<=40){
			$('.btn1').text('好棒！截图微信领红包').removeClass('share').addClass('playagain');
			user = 40;
		}
		...
		$('#sscore').text(score);
		$('#suser').text(user+'%');
		//.removeClass('playagain').addClass('share');
	},
	isMobile : function(){
